...
type LibraryDocument = {
  ...
  previewUrl?: string
  renderMode?: 'text' | 'markdown' | 'docx' | 'pdf' | 'image' | 'embed' | 'unsupported'
}
...
function renderMarkdownPage(text: string) {
  return (
    <div className="atop-doc-markdown-rendered">
      <ReactMarkdown>{text}</ReactMarkdown>
    </div>
  )
}

function renderDocumentPage(doc: LibraryDocument, text: string, side: 'left' | 'right'): ReactNode {
  if ((doc.renderMode === 'docx' || doc.renderMode === 'pdf') && doc.renderedPages?.length) {
    const pageMarkup = doc.renderedPages[Number(text)]
    if (!pageMarkup && side === 'right') {
      return <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="已到末页" />
    }
    if (doc.renderMode === 'pdf') {
      return (
        <div className="atop-doc-pdf-rendered">
          <img src={pageMarkup} alt={`${doc.title} 第 ${Number(text) + 1} 页`} />
        </div>
      )
    }
    return <div className="atop-doc-office-rendered" dangerouslySetInnerHTML={{ __html: pageMarkup ?? '' }} />
  }
  if (doc.renderMode === 'markdown') return renderMarkdownPage(text)
...
function createPageNode(doc: LibraryDocument, text: string, pageNumber: number) {
  const wrapper = document.createElement('div')
  wrapper.className = `atop-doc-book-page atop-doc-kind-${doc.kind}`
  wrapper.dataset.density = 'soft'
  wrapper.innerHTML = `
    <div class="atop-doc-page-number">${pageNumber}</div>
    <div class="atop-doc-page-kind"><span>${KIND_LABEL[doc.kind]}</span></div>
  `
  const body = document.createElement('div')
  body.className = 'atop-doc-page-print'
  if (doc.renderMode === 'docx' && doc.renderedPages?.[Number(text)]) {
    body.className = 'atop-doc-office-rendered'
    body.innerHTML = doc.renderedPages[Number(text)]
  } else if (doc.renderMode === 'pdf' && doc.renderedPages?.[Number(text)]) {
    body.className = 'atop-doc-pdf-rendered'
    const img = document.createElement('img')
    img.src = doc.renderedPages[Number(text)]
    img.alt = `${doc.title} 第 ${pageNumber} 页`
...
async function renderDocxPages(file: File) {
  const container = document.createElement('div')
  await renderAsync(file, container, undefined, {
    breakPages: true,
    className: 'atop-docx-preview',
    inWrapper: false,
    renderHeaders: true,
    renderFooters: true,
  })
  const pages = Array.from(container.querySelectorAll<HTMLElement>('.docx-wrapper > section, section.docx, article, section'))
  const nodes = pages.length > 0 ? pages : Array.from(container.children) as HTMLElement[]
  return nodes
...
export default function DocumentCenterPage() {
  ...
  const flipHostRef = useRef<HTMLDivElement | null>(null)
  ...
  useEffect(() => {
    if (!selected || !flipHostRef.current) return
    const host = flipHostRef.current
    host.innerHTML = ''
    const pageNodes = selected.pages.map((page, index) => createPageNode(selected, page, index + 1))
    if (pageNodes.length === 0) return
    try {
      flipRef.current?.destroy?.()
      host.innerHTML = ''
      pageNodes.forEach((node) => host.appendChild(node))
      flipRef.current = new PageFlip(host, {
        width: 420,
        height: 560,
        size: 'stretch',
        minWidth: 300,
      ...
      flipRef.current.loadFromHTML(pageNodes)
      flipRef.current.on('flip', (event: { data: number }) => {
        setPageIndex(Math.max(0, event.data ?? 0))
      })
    } catch (error) {
      console.warn('page-flip 初始化失败，回退到普通双页布局', error)
    }
    return () => {
      flipRef.current?.destroy?.()
  ...
  }, [selected?.id, selected?.pages.length])
  ...
  const handleUploadFile = async (file?: File) => {
    ...
    let pages: string[] = []
    let renderedPages: string[] | undefined
    ...
    try {
      if (kind === 'markdown') {
        renderMode = 'markdown'
        pages = splitTextPages(await file.text(), 980)
      } else if (ext === 'txt') {
        pages = splitTextPages(await file.text(), 720)
      } else if (kind === 'image') {
        previewUrl = URL.createObjectURL(file)
        renderMode = 'image'
        pages = [`${file.name}\n图片已在当前页真实预览。`]
      } else if (kind === 'pdf') {
        renderedPages = await renderPdfPages(file)
        renderMode = renderedPages.length > 0 ? 'pdf' : 'unsupported'
        pages = renderedPages.length > 0
          ? renderedPages.map((_, index) => String(index))
          : [`${file.name}\nPDF 文件已识别，但渲染失败。建议改用后端页级转码生成稳定预览。`]
      } else if (kind === 'word' && ext === 'docx') {
        renderedPages = await renderDocxPages(file)
        renderMode = renderedPages.length > 0 ? 'docx' : 'text'
        pages = renderedPages.length > 0 ? renderedPages.map((_, index) => String(index)) : await parseOfficeFile(file, kind)
      } else if (kind === 'ppt' && ext === 'pptx') {
        pages = await parseOfficeFile(file, kind)
        if (pages.length === 0) {
          renderMode = 'unsupported'
          pages = [`${file.name}\npptxjs 依赖已加入，但当前 npm 包没有可用浏览器渲染入口。PPT 原样预览建议走 LibreOffice/OnlyOffice 转 PDF 或图片后再翻页。`]
        }
      } else {
        renderMode = 'unsupported'
        pages = [`${file.name}\n该格式需要后端转码服务后才能完整预览。`]
            ...
            <>
              <div className="atop-doc-reader-head">
                <div>
                  ...
                  <p>
                    {selected.renderMode === 'docx' && 'DOCX 使用 docx-preview 按 Office 文档结构渲染，分页后交给真实翻页引擎展示。'}
                    {selected.renderMode === 'pdf' && 'PDF 使用 pdfjs-dist 渲染为页图，再交给真实翻页引擎展示。'}
                    {selected.renderMode === 'markdown' && 'Markdown 使用 react-markdown 渲染为阅读版式，不再显示原始源码。'}
                    {(!selected.renderMode || ['text', 'image', 'embed'].includes(selected.renderMode)) && `当前仅渲染第 ${pageIndex + 1}-${Math.min(pageIndex + 2, totalPages)} 页，翻页时预留相邻页缓存位。`}
                    {selected.renderMode === 'unsupported' && '当前格式需要后端转码服务才能做到原样预览。'}
                  </p>
                </div>
              ...
              <div className={`atop-doc-book atop-doc-book-${selected.kind} ${turning ? `is-turning-${turning}` : ''}`}>
                <div ref={flipHostRef} className="atop-doc-flip-stage" />
                <button
                  type="button"
                  className="atop-doc-page-hit atop-doc-page-hit-left"
                ...
                <button
                  type="button"
                  className="atop-doc-page-hit atop-doc-page-hit-right"
                  disabled={pageIndex >= totalPages - 2 || Boolean(turning)}
                  onClick={() => turnPage('next')}
                  aria-label="翻到下一页"
                />
                <div className="atop-doc-fallback-pages" aria-hidden="true">
                  <div className="atop-doc-book-page atop-doc-book-page-left">
                  ...
                  <div className="atop-doc-book-page atop-doc-book-page-right">
